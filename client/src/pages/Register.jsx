...
export const Register = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      // Register via backend — returns { success, token, user }
      const response = await registerAPI({
        ...form,
        year: form.year ? parseInt(form.year, 10) : undefined,
      });

      // Store token and push user into AuthContext directly
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-950 py-12 px-4 sm:px-6 lg:px-8 font-sans">
      <div className="w-full max-w-lg p-8 bg-slate-900/60 backdrop-blur-md rounded-2xl shadow-2xl border border-slate-800">
        ...
        <div className="flex flex-col items-center mb-8">
          ...
          <p className="mt-2 text-[0.95rem] text-slate-400 text-center">
        ...
        <form className="space-y-4" onSubmit={handleSubmit}>
          {/* Role selector — drives which fields are shown */}
          <div>
            ...
            <div className="grid grid-cols-2 gap-3">
              {['student', 'admin'].map((r) => (
                ...
                >
                  {r === 'student' ? '🎓 Student' : '🛡️ Admin / Faculty'}
                </button>
              ))}
            </div>
          </div>
          ...
          <div>
            ...
            <input
              ...
              className="w-full px-3 py-2 bg-slate-800/50 border border-slate-700 rounded-md text-[0.95rem] text-white focus:outline-none focus:ring-1 focus:ring-cyan-500 focus:border-cyan-500"
            />
          </div>
          ...
          {form.role === 'student' && (
            <>
              <div className="grid grid-cols-2 gap-4">
                <div>
                  <label htmlFor="studentId" className="block text-sm font-medium text-slate-300 mb-1">
                    Student ID
                  </label>
                  <input
          ...
          <Button type="submit" className="w-full py-2.5 text-base !mt-6" isLoading={isSubmitting}>
            Create Account
          </Button>
        </form>
